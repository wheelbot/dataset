...
if __name__ == "__main__":
    ...
    next_states = jnp.array(data["nextstates"])[:, :MAX_ROLLOUT_LENGTH, :]
    ...
    # next_states_norm = next_states_norm - states_norm[:, None, :]
    
    n_samples = len(states_norm)
    key = jax.random.PRNGKey(0)
    indices = jax.random.permutation(key, n_samples)
    
    split_idx = int(0.8 * n_samples)
    train_dataset = DynamicsDataset(
        states_norm[indices[:split_idx]],
        actions_norm[indices[:split_idx]],
        next_states_norm[indices[:split_idx]],
        trajectory_length=MAX_ROLLOUT_LENGTH
    )
